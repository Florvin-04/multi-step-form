...
function setTheCheckedPlanInput() {
  if (localStorage.plan) {
    plans.forEach((plan) => {
      const planObject = localStorage.plan;
...
function setTheCheckedAddons() {
  if (localStorage.addonsList) {
    listOfAddons = localStorage.addonsList;
...
function nextButton() {
  nextBtn.addEventListener("click", () => {
    ...
    if (!validateFields()) {
      ...
      moveToStep(stepNumbers, currentStepNumber, stepNumbers[parseInt(currentStepNumber.dataset.index) + 1]);
      checker(targetStep);
...
function getPlanInfos() {
  const planHolder = document.querySelector("#plan-holder");
  const planPrice = document.getElementById("plan-price-holder");
  const plan = localStorage.plan;
  planSubscription = localStorage.billing_type;
  planHolder.textContent = `${plan.name} (${planSubscription})`;
  planSubscription == "Monthly" ? (planPrice.textContent = `$${plan.price}/mo`) : (planPrice.textContent = `$${plan.price * 10}/yr`);
}
...
function getTotal() {
  ...
  planSubscription = localStorage.billing_type;
  if (localStorage.addonsList) {
    theAddonsList = localStorage.addonsList;
  ...
  planSubscription == "Monthly" ? (totalType.textContent = "Total (Per Month)") : (totalType.textContent = "Total (Per Year)");
  planSubscription == "Monthly" ? (totalPrice.textContent = `+$${total}/mo`) : (totalPrice.textContent = `+$${total * 10}/yr`);
}
...
function showMessageOfThanks() {
  ...
  if (window.innerWidth > 768) {
    ...
    steps[steps.length - 1].style.trans.late = "0 -50%";
  }
}
